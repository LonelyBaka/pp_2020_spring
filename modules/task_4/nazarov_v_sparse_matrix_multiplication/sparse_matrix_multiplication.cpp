...
#include <set>
#include <utility>
#include "../../../modules/task_4/nazarov_v_sparse_matrix_multiplication/sparse_matrix_multiplication.h"
...
CRS_Matrix CRS_Matrix::parallelMultiply(const CRS_Matrix& mat) const& {
    ...
    for (size_t i = 1; i < rowIndex.size(); ++i) {
        ...
        for (int threadIter = 0; threadIter < threadNum; ++threadIter) {
            threads[threadIter] = std::thread([&](int begin, int end, const int threadId) {
            ...
            }, threadIter * blockSize + 1, threadIter * blockSize + blockSize + 1, threadIter);
        }
